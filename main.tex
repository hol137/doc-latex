...
The Particle Physics must unlock by according Eddington's, Wyler's, Atiyah's and Sternheimer's approaches to Holic Principle, 
with emphasis on the $10^{61}$ Trans-Plankian Quantized time, the Kotov's period, the Tifft's speed, the Koide-Sanchez constant, the Brannen matrix, the massive gluons and composite d quark. Cosmology must unblock by oberving mature galaxies instead of that 'dark space' predicted by using the imperfect cosmological principle.  
...
With $\delta^{\prime} = p_K^2/a^3 $, and the symmetrised Planck area $(l_P^{\prime})^2 = h\hbar G/c^3$ (-11, 11 ppm, and 0.04 \%):
\begin{equation}
(R_{GC}/l_P^{\prime})^4/\sqrt{\pi} \approx \mu^{210} \approx (O_M \delta^{\prime})^9 \approx \tau^{137}
\end{equation}
The existence of a Final Theory based on the Holic Principle (section 6) and the Koide-Sanchez constant cannot be denied. The interpretation is clear: the 4D space-time in Grandcosmos is associated with a 9D space involving the Monster. \textit{This opens a path towards the Final Theory}. 
