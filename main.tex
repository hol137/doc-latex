...
The antique concept of permanent Cosmos is reintroduced as a perfect deterministic Computer, inverting the anthropic principle, and interpretating the physical dimensionless parameters as optimal calculation basis. The later are unified in the Topological Axis, which rehabilitates the string theory dimension series 4k+2, with emphasis on the values 26 (visible Universe) and 10 (Hydrogen - Pion couple). The 1D extension of the holographic principle, defining Grandcosmos, breaks the Planck wall by a factor $10^{61}$, resolving the vacuum energy dilemma. 
...
The mathematics-physics fusion is confirmed by $10^{-9}$ precise relations, implying the Eddington's constant 137 and the Atiyah's constant. Quantized time, Kotov's period, Koide relation, massive gluons and composite d quark will deblock Particle Physics while mature galaxies will be found in the far-field, with no dark space, excluding the standard evolutionary cosmology, ill-founded on the imperfect cosmological principle.
...
~~~    7.1 the CosmicSpecial Holographic Relation
   
~~~    7.2. Conservation of Information
...
~~~    9.4. Electroweak-Gravitational ppb Fine Tuning
...
properties of a coherent wave. Now, the main lesson of modern physics is that everything (light and matter) propagates by wave. (quanta appearing only et the detection). Moreover a coherent wave is represented by a unitary matrix of quantum mechanism. It has been shown that the quantum formalism is very similar to the holographic one, describing an intreaction bu a two-step holographic process. From these consideratiosn a mass for the photon (confirmed in section) and graviton has been proposed \cite{Sanchez1}, and are in definite places in the Topological Axis (fig. 1)

A Holographic Principle has been coined by some theoricians \cite{Bousso}, the name coming from the idea of dimension reduction, from 3D to 2D, similar to the visual impression in current visible holograms (in fact there is only the 2D restitution of a propagating wave). It this respect, it is strange that no one tried to extend this process to 1D (an idea proposed in the author's thesis in 1975): this is done in Section 3. This means that the Planck length is not the single holographic wavelength. In fact \textit{the Topological Axis is the reunion of height 1D-2D holographic relations}. We present here two significant examples. 
...
\subsection {The 4 Forces ppb Fine-Tuning}
The Particle standard model achieved the unification between electromagnetism and weak nuclear force, with propositions for extension towards strong nuclear force in Grand Unification Theory (GUT), but no synthesis have been possible with gravitational force. The Topologocal axis shows clearlty that GUT with $1O^{16}$ GeV gauge boson seems confirmed. However, in section 4.2, it is proven that the Kotov oscillation reveals a symmetry between two apparently inconciliable domains: electroweak and gravitational forces. So we look here for a precise relation involving the 4 force parameters, $a$ (electric), $aw$ (weak nuclear), $f$ (strong nuclear) and $a_G$ (gravitation). The later force is equivalently represented by $p_G$ = P/2^{127/2}$, where $P = m_P/m_e$, which was admitted to be:
\begin{equation}
...
\end{equation}
 ...
 Now $a_{w} is a cube: $a_{w} = (\lambdabar_{e}/l_{eF})^{3}$, with $l_{eF} = (G_{F}/m_{e} c^{2})^{1/3}$. This writes    
\begin{equation}
...
\end{equation}
This relation is so striking that we ask the computer if it can find synthetic precise relations involving the other forces parameters, in function of involving $\pi$, $gamma$, the Atiyah constant $\Gamma = \gamma a/\pi$, and the Eddington's constant 137. Indeed, whith the neutron/electron mass ratio $n^$ \approx 1838.68366089(17)$, the computer gives the following relation in the ppb range:
\begin{equation}
aF/\sqrt(pH) \approx \pi(4n//DF\gamma)^3/pG \approx \mu^2
\end{equation}
where $\m2$ is the Muon/electron mass ratio, textit{inside its 40 ppb indetermination}, so proposing the value:
\begin{equation}
\mu \approx 206.7682869
\end{equation}

Note that $n/\Gamma$ is close (3.4 ppm) to the monstrous 5th term 292.6345909 in
...
\end{equation}
The following weak boson ratios: 
\begin{equation}
...
\textit{The very existence of so precise ppb relations confirms there exists a Final}. The following section indicates hints towards this Final Theory.


\subsection {The Heavy Leptons Fine-Tuning}
The standard model is unable to explain why there are three famillies of particle. thus the study of the muon and tau mass ratioes is crucial. The study of Kotov period shows. Now, the Koide relation \cite{Koide}, which has a mathematical justification in term of circulating matrix, correctly predicted $\tau$ at an epoch during which its measurement was false to 3 $\sigma$. It can be wrote:
\begin{equation}
(1 + \mu + \tau)/2 = (1 + \sqrt\mu + \sqrt\tau)^2/3 = p_K
\end{equation}
Now the Koide constant $p_K \approx 1842.604994$ verifies (2 et 0.16 ppm):
\begin{equation}
p_K^4 \approx (4\pi)^4 apH \approx (pH)^2 (137a/136(a^{\prime}-1) d_e
\end{equation}
With the above $\mu$ value, $\tau \approx 3477.441701$, which obeys, to 0.7 and -0.8 ppm:
\begin{equation}
$$e^{e^e}/a e^e \approx (F/137)^{3}/ (\mu\tau/j)^2 \approx  \sqrt{pH} (p/p_0)^2$$ 
\end{equation}
resulting from the elimination of $a_m = \sqrt{137a}$ in the following extension of the above relation defining $\mu$, giving a central role to the Tifft ratio $F/137$, and so defining the Brout-Englert-Higgs ratio $s$:
$$
\begin{array}{ll}
%
\displaystyle
...
%\bibitem{Grosmann} Grosmann, Michel and Meyrueis, Patrick. ``Optics and Photonics Applied to Communication and Processing''. 01.1979
