...
~~~    9.4. Electroweak ppb Fine Tuning
   
~~~    9.5. Heavy Leptons ppb Fine Tuning
   
~~~    9.6. Gravitation ppb Fine Tuning
...
The holograpphy technique is by far the most efficient way to treat huge information, based on the
properties of a cohernt wave. Now, the main lesson of modern physics is that everything (light and matter) propagates by wave. Moreover a coherent wave is represented by a unitary matrix of quantum mechanism. It has been shown that the quantum formalism is very similar to the holographic one, describing an intreaction bu a two-step holographic process. From these consideratiosn a mass for the photon (confirmed in section) and graviton has been propoesed \{citeSanchez1}, and are in definite places in the Topological Axis (fig. 1)

A Holographic Principle has been coined by some theoricians \{citeBousso}, the name coming from the idea of dimension reduction, from 3D to 2D, a visual impression in current visible holograms (in fact there is only the 2D restitution of a propagating wave). It this respect, it is strange that nobody tried to extend this process to 1D (an idea proposed in the thesis of the main author in 1975): this is done in Section 3. But the holography must use different wavelength than $l_P$. In fact the Topological Axis is the reunion of height 1D-2D holographic relations. We present here two significative examples 
...
\subsection {The Electroweak ppb Fine-Tuning}
The Particle standard model achieved the unification between electromagnetism and weak
nuclear force. Looking for relations involving $a_{w}$ and the intermediate bosons ratios $W$ et $Z$, the computer indicates: 
\begin{equation}
a_{w} = (137 \times 2 \Gamma)^{3}
\end{equation}
or, equivalently, with the length $l_{eF} = (G_{F}/m_{e} c^{2})^{1/3}$, the electroweak constant appears as
...
\end{equation}
\textit{within its 0.25 ppm indetermination}, confirmed, \textit{again in the ppb range} by:
\begin{equation}
( \Gamma F/\gamma d_{e})^{1/3} \approx 4n^{\prime} / \Gamma
\end{equation}
where $n^{\prime} = m_n/m_e^{\prime} = nH/p$ is the effective neutron mass ratio, with the neutron/electron mass ratio $n \approx 1838.68366089(17)$. \textit{Final theory cannot avoid these relations} 
...
\subsection {The Heavy Leptons ppb Fine-Tuning}
The standard model is unable to explain why there are three famillies of particle. thus the study of the muon and tau mass ratioes is crucial. The study of Kotov period shows: 
\begin{equation}
E_{F}/m_{e} c^{2} \approx
m_{\mu}^{2}(m_{p} m_{H})^{1/2}/am_{e}^{3}
\end{equation} 
With the above value for $F$, this yields to: 
\begin{equation}
\mu \approx 206.7682869
\end{equation} 
\textit{within its 20 ppb measurement range}.
Now, the Koide relation \cite{Koide}, which has a mathematical justification in term of circulating matrix, correctly predicted $\tau$ at an epoch during which its measurement was false to 3 $\sigma$. It can be wrote:
\begin{equation}
...
\subsection {The Gravitational ppb Fine-Tuning}
...
\end{equation}
Note that this is close (0.12\%) to the monstrous 5th term 292.6345909 in
