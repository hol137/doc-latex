...
The Particle standard model achieved the unification between electromagnetism and weak nuclear force, with propositions for extension towards strong nuclear force in Grand Unification Theory (GUT), but no synthesis have been possible with gravitational force. The Topologocal Axis shows clearly that GUT with $1O^{16}$ GeV gauge boson seems confirmed. Moreoverer, in section 4.2, it is proven that the Kotov oscillation reveals a symmetry between two apparently inconciliable domains: electroweak and gravitational forces. So we look here for a precise relation involving the 4 force parameters, $a$ (electric), $a_w$ (weak nuclear), $f$ (strong nuclear) and $a_G$ (gravitation). The later force is equivalently represented by $p_G = P/2^{127/2}$, with $P = m_P/m_e$, which was defined in section 2.4 by the two first terms of the \textit {ppb range holic series}:
\begin{equation}
...
\end{equation}
This value is confirmed (30 ppb), with the above tachyonic ratio $Q$ and the Planck Wien term $5/w$ (section 7.1) (3\%, 1\%, 60 ppb) 
\begin{equation}
Q \approx P^e \approx W^{\sqrt a} \approx 48 \sqrt{5/w}~ O_M Z
\end{equation}
the Z value is also confirmed by its ppb liaison with the Janko pariah group order $J_3 = 50232960 $ \cite{Janko}
\begin{equation}
J_3 \approx 3\pi^2 Z^2 / (137^2 + \sqrt{1+d_e})
\end{equation}
There is a liaison between the monster and the $J_3$ orders, while these two groups have no known relations (0.2 ppm):
\begin{equation}
...
\end{equation}
This Koide-Sanchez constant, that no one noticed, $p_K \approx 1842.604994$ has been detected in the above study, leading to (2 ppm, with an additive term at 0.16 ppm):
\begin{equation}
...
\end{equation}
resulting from the elimination of $a_m = \sqrt{137a}$ in the following extension of the above relation defining $\mu$. This gives a central role to the Tifft ratio $F/137$, and so proposing a Brout-Englert-Higgs ratio $s \approx 495.88^2$, corresponding to 125.65 GeV for the mass ratio Brout-Englert-Higgs-Boson/Electron. Also, in the Topological Axis, it corresponds with $k \approx \pi $, and $n \approx \gamma \Gamma$ (Fig. 1).
$$
\begin{array}{ll}
%
\displaystyle
...
$$ F/137 \approx \Pi_{par}^{1/20} \approx f O_M^{1/20}(a/137)^{1/10}\beta^{1/5}$$ 
 ...
 With the classical electron radius $r_e = \lambdabar_e/a$, $p_K$ enters Eq.(31) (0.5, 17 and 40 ppm):
\begin{equation}
r_{e}/\lambdabar_M \approx(4\pi P/p_K)^4 \approx (R_1l_K/R^{\prime}r_e)^3 \approx (es)^{15}/p^2  
\end{equation}
confirming the above BEH ratio $s$ and relying with the Monstrer orders product $O_MO_B/\sqrt{2}$ (see Eq.(54)).
