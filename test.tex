...
$$
very close (0.4 ppm) to $R_{1} = ( p_{G} /p_{0} )(\beta R R^{\prime})^{1/2}$, where $p_{G} = P/2^{127/2}$, with $P = \lambdabar_e / l_P$, $\beta = (H - p)^{-1}$ is the Rydbergh correction factor and $p_{0} = 6\pi^{5}$ is the Lenz-Wyler value $p$ (Section 9.2):
. Moreover, there is a direct connection with the Grandcosmos radius and the topon, to 0.90 \%: $R_1^3/R_{GC}^2 \approx \lambdabar_M$. The analysis of deviation leads to (0.3 ppm):
\begin{equation}
