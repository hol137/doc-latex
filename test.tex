...
Note that the measured proton 'charge radius' $8.775(5)\cdot 10^{-16}~m$ is slightly distinct from the above value. There is presently a 'proton radius puzzle'\cite{Carlson}. With the Plank density $d_{P}$ and $v_p$ the proton volume, the following relation is very intriguing\cite{Haramein}:
\begin{equation}
d_Pv_P \approx M_B
\end{equation}
where $M_B \approx 0.04 \cdotM$ is the baryon mass of the visible Universe.
