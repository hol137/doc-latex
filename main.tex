...
the deviance $R/(2\sqrt{(M/m\prime_{e}))}$ to the Hydrogen wavelength $\lambdabar_{H} = \hbar/c \c m_{H}$, one obtains the double relation:
\begin{equation}
R/2\lambdabar_{H} = \sqrt{(M/m\prime_{e})} = \frac{\hbar \cdot c}{G \c m_{e}\c m_{p}}
\end{equation}
