...
%So, this comes back to the antique problem of the rationalisation of $\sqrt{2}$. Its corresponding value shows a $%8^{th}$ fractional term very close to $\sqrt{5}$, so leading to $\sqrt{2}_a \approx N_{a}/D_{a}$, with $ N_{a} %= %239 \sqrt{5} + 99 \approx \Gamma^{2}
...
$p \approx \Gamma (\pi e)^2$. There is a quasi-holographic relation connecting the constants $e$, $\pi$ and $\gamma$ and the volume $v_2 = (4\pi/3) (ln2)^3$ (0 ppb and 0.046 \%):
\begin{equation}
e^{1/3} = (4\pi/3)\gamma^2 \approx v_2 
\end{equation}
$v_2$ is tied to $a$ through (354 ppb):
\begin{equation}
v_2^{16} \approx 3a^2/2\times 137 
\end{equation}
This was found from $\mu \approx 3a/2 \approx e^{16/3}$, section 9.5.

\subsection {The optimal calculation base $e$ confirmed}

The electron magnetic momant $2d_e$ appears in (0.7 ppm):
\begin{equation}
...
$$ X \approx (upn^2)^2 \approx (d_e \beta H^2/u)^3$$ close to the cubic root of the Systema number \cite{Moulin} $N_{sys} = 2^{65}\cdot 3^{41} \cdot 5^{28}$. The deviation reveals the Wyler's form $p_{aW} = 6 (a^2 - 137 ^2)^{5/2}$ (16 ppm):
$$ X^3 \approx N_{sys} p_{aW} /a^{3/2}$$
This term appears also in the following proposition for the weak mixing effective angle 0.23149(13), where $\Phi$ is the golden number (3.6, 30 and 8.5 ppm):
\begin{equation}
(1/ln\Phi)^2 \approx g_0/p_{aW} /approx 2 sqrt{s}/e^{2e} /approx 16 \times 496 H/pn  
\end{equation}
...
Since $R/\lambdabar_e \approx 2^{128}, $the \textit{the decomposition of 6 leads to a natural Universe-Grandcosmos partition}, and to the following approximation for the tachyonic celerity ratio (0.01\%) 
$$ U = C/c\approx3^{128} (p_K/p_G \delta)^2 $$ where $p_K$ is the Koide-Sanchez constant (see section 9.5). This confirms the role of the correspondance quark up = 5 and \textit{quark d = 6 with a double structure}. It is an example of \textit{immergence}, i.e. deducing the small from the large, in a striking similitude between cosmology and nuclear physics. Another example was encontered in section 2.4, where dimensional analysis gives the visible Universe radius, in an easier way than the equivalent one for the Hydrogen atom radius, since for this case there is no evidence that $c$ must be left out. Another example signals a general misconception: the coherence of the stimulated emission in a laser is a \textit{global effect] in a homogeneous media (atomic coherence). This elimination of $q$ leads to (2.6 \%):
...
\end{equation}
confirming the role of the Planck volume and the Kotov length.
...
\end{equation}
where $p_K$ is the Koyde-Sanchez constant (section 9.5), and $p_{moy}^3 = n p_a p_E$, with the neutron/electron mass ratio $n \approx 1838.68366089(17)$, $p_a = (4\pi )^2 \sqrt{a}$ and $p_E = 1847.599459$, the Eddington's value.  
...
\end{equation}
leading to $F = a_{w}^{1/2} = E_{F} /m_{e} c^{2} \approx 573007.3652$. Moreover, with the definition of the Bizouard strong force parameter $f$:
\begin{equation}
...
\end{equation}
Note that this value $f$ is close (0.06 \%) to the following double conjonction (7 and 5 ppm):
\begin{equation}
(a-135)^3 \approx (a-2)/16 \approx \pi^4 ln2/8 
\end{equation}


Note that $4n/\Gamma$ Iin Eq. (64) is close (3.4 ppm) to the monstrous $5^{th}$ term 292.6345909 in
...
\textit{in the ppb range}. Firstly: 
\begin{equation}
...
\end{equation}
confirming the holic attribution (section 6) of power 7 for a field parameter, and the central role of the dimension 496. Now:
\begin{equation}
...
\end{equation}
From $ J_3 \approx W^2/496$, the Eq (69) induces a liaison between the monster and the $J_3$ orders, while these two groups have no known relations (0.2 ppm):
\begin{equation}
O_M \approx d_e J_3^7 \sqrt(p/p_0)
\end{equation}
\textit{The very existence of so precise ppb relations confirms there exists a Final Theory involving the sporadic groups}. The following section shows additive hints.
