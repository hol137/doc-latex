...
The electromagnetic interaction is not really understood \cite{Okun}. Consider for simplicity two identical system of mass m in their basic state. They are each characterized by a stationnary wave, which may be seen as the sum of a diverging wave and a converging one: s + s*, with s = exp(if2p(t-r/c)), where f is the proper frequency mc2/ħ. The second system is characterized by the analogous wave r + r*. Supposing that the vacuum is not empty, an hologram is formed: (s + s*)(r + r*), which includes the resonant terms sr* + s*r. So, the simple presence of two systems create such an inhomogeneity in all the Universe.  Now if the first system has an excess of energy, this means it is receiving an excess signal of the form s*. By diffraction on the above hologram it gives rise to s*(sr* + s*r), with resonant term r*. So convergent waves are of primordial importance, instead of current diverging ones. But the process is symetrical, so this leads to a oscillation. This is known a the particle exchange of a massive boson associated with any interaction. Here, it is assumed here that the boson has a tachyonic speed $C_1$. Now, the resonance condition is that the wavelengths are identical, by analogy with the Gabor condition\cite{Gabor}. So, for the electron wave and the weak wave:
\begin{equation}
...
\end{equation}
Equaling the tachonic celerities to C_1, and $ m_{ph}$ with $\hbar/cl_K$ (cite{Sanchez1}, one gets a \textit{ppb correlation} with the above single electron radius R_1:
\begin{equation}
...
relation with the CMB Wien wavelength $l_{CMB} = hc/kTv $, with $v = 5 (1-e^{-v}) \approx 4.965114245)$, and the proton radius, identified to $\lambdabar_e/\sqtr{D} \approx 8.7029\cdot 10^{-16}$ m (0.1 \% and -0.1 \%):
\begin{equation}
...
\bibitem{Aschbacher} Aschbacher H. Finite Group Theory. C.U.P. 242-248 (2000). 
...
\bibitem{Witten} Witten E. The Three-Dimensional Gravity Revisted. arxiv.org/abs/076.3359

\bibitem{Sanchez5} Sanchez F.M. Electrical Moonshine. vixra:1801:0067 
