...
conjonction:
$$f{e^{2}} = exp(2 e^{2} + 1⁄2 ) \approx exp(e^{2e} + e^{2} )$$
...
immediately gets:
$$a_w \approx (2\gamma \cdot 137 \cdot a/\pi)^3$$
Now, by introducing the featured length $$l_{eF} = (G_F /m_e c 2 )^{1/3}$$ , the electroweak constant appears as
...
development of $\pi$ is always a non-resolved problem, this confirms that present mathematics is
