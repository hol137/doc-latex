...
The Particle Physics must unlock by according Eddington's, Wyler's, Atiyah's and Sernheiemer's approches to Holic Principle, 
with emphasis on the $10^{61}$ Trans-Plankian Quantized time, the Kotov's period, the Tifft's speed, the Koide-Sanchez constant, the Brannen matrix, the massive gluons and composite d quark. Cosmology must deblock by the oberving mature galaxies instead of that 'dark space' predicted by using the imperfect cosmological principle.  
...
The fact that three parameters, among of about 30, are so clearly emerging means that physics, and more generally science, follows a hierarchical order: \textit{one can progress without knowing the details of the underlying fundamental theory.}
...
In the same manner, this article prefigures \textit{the fundamental theory, but precizing its arithmetical foundation: the Holic Principle}, recalled in section 6. Thus, there must exist multi-base algorithms able to explain the compatibiloity between these two principles, which seems at first sight contradictory. 
...
Note that the above Wien length enters (O.O3 \%):
\begin{equation}
l_{CMB} / \lambdabar_e \approx P/a^3 p H 
\end{equation}
This confirms the invariance of the CMB Temperature.  
...
The cardinal orders of the Monster and the Baby-Monster groups enter the supercelerity ratio $Q = R_{GC}/R = C/c$, where $D = 196883$ is the Monster dimension (63  and 2 ppm)
\begin{equation}
...
\end{equation}
This value is confirmed by the 30 ppb relation:
\begin{equation}
Q = C/c \approx 48 O_M (5/w)^{1/2}
\end{equation}
confirming the role of the Planck thermal law.
The Z value is also confirmed by its ppb liaison with the Janko pariah group $J_3$
\begin{equation}
...
From the observation (0.04 \% and 3\%):
\begin{equation}
\tau^{137} \approx \mu^{210} /4\pi \approx O_M^9
\end{equation}
the Extended Holic Principle (section 6) cannot be denied. Moreover, with $\pi_0 = (7(35^2-1)-(2^7-1)/3 \times 7 \times 2^7)$, where the Combinatorial Hierarchy terms appear, the deviation reduces to 112 ppb; \textit{the intervention of $\pi$ - rationalisation cannot be denied}.
...
\item 1/ The CMB temperature will show invariance
...
\bibitem{Brannen} C.A. Brannen, The Lepton Masses, http://brannenworks.com/MASSES2.pdf, (2006).
...
\bibitem{Sanchez4}Sanchez F M (1995), Holic Principle, ANPA Proc. (Cambridge) 16, 324
