...
\textit{With unit the Electron Compton wavelength, in the macro-physics side}, the Universe circumference 
    ...
    the Kotov length, through a factor 2$\pia$, with $a \approx 137.036$. For $n \approx \Gamma $, the Atiyah constant, it means the galaxy group radius, a characteristic cosmic length ($10^{6}$ ligth-years). For $k \approx e^{2}$, $y \approx 2e$, it is the Grandcosmos radius.
