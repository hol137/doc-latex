...
and results from the imbrication of relations of the form $\lambdabar_{e} /l_{micro} \sim (l_{macro} /\lambdabar_{e})^{2} $, followed by $l_{macro} /\lambdabar_{e} \sim
...
where $l_{P} = (G\hbar/c^{3} )^{1/2}$ is the Planck's length. Note that, while the standard evolutionary cosmology
...
292.806161(6) GeV \approx 573007.33(25) m_{e} c^{2}$ , itself tied to the weak force constant 
\begin{equation}
G_{F} \equiv (\hbar c)^{3} /E_{F}^{2} \approx
1.4358509(7) \times 10^{-62} Joule \cdot m^{3}
\end{equation}
...
\end{equation}
where j is the Sternheimer scale factor, central in Theoretical Biology [3][35], being, in particular the
