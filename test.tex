...
\end{equation}
Equaling the tachonic celerities to $C_1$, and $m_{ph}$ with $\hbar/cl_K$ (cite{Marchal}, one gets a \textit{ppb correlation} with the above single electron radius $R_1$, taking account of the above relation tying $R_1$ and $l_K$(section 4.1):
\begin{equation}
C_1/c = m_e/m_{gr} = m_w/m_{ph} = l_K/\lambdabar_w \approx R_1/\lambdabar_e (4\pi p/p_0)^2 
\end{equation}
This leads to $m_{ph} \approx 1.2222 10^{-55}$ kg, and  $m_{gr} \approx 3.7223 10^{-67}$ kg, which fit canonical places in the Topological Axis (Fig 1). This means (0.8 \%):
\begin{equation}
